...
export const updateUserProfile = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (updateData.hasSmartphone !== undefined) {
      updateData.hasSmartphone = updateData.hasSmartphone === 'true' || updateData.hasSmartphone === true;
    }
    ...
    if (updateData.preferredSubjects && !Array.isArray(updateData.preferredSubjects)) {
    ...
    if (req.file) {
      ...
      if (existingUser && existingUser.profileImage) {
        const oldImagePath = path.join(process.cwd(), 'public', existingUser.profileImage.replace('/uploads', '')); // Adjust path if needed
