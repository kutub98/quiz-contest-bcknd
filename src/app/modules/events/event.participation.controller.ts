...
export const registerForEvent = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!event || !event.isActive) {
      return res.status(404).json({ success: false, message: 'ইভেন্ট পাওয়া যায়নি বা নিষ্ক্রিয়' });
    }
    
    if (event.status === 'completed') {
      return res.status(400).json({ success: false, message: 'ইভেন্ট ইতিমধ্যে শেষ হয়েছে' });
    }
    
    if (event.participants.includes(userId as any)) {
      return res.status(400).json({ success: false, message: 'আপনি ইতিমধ্যে এই ইভেন্টের জন্য রেজিস্টার্ড' });
    }
    ...
    res.json({ success: true, message: 'ইভেন্টের জন্য সফলভাবে রেজিস্টার্ড হয়েছেন' });
  } catch (error: any) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
export const getEventQuizzesForStudent = async (req: AuthRequest, res: Response) => {
  try {
    const { eventId } = req.params;
    const userId = req.user?.userId;
    ...
    if (!event || !event.participants.includes(userId as any)) {
      return res.status(404).json({ success: false, message: 'ইভেন্ট পাওয়া যায়নি বা রেজিস্টার্ড নয়' });
    }
    ...
    const availableQuizzes = quizzes.filter(quiz => 
      !user?.participatedQuizzes.includes(quiz._id)
    );
...
export const getStudentEventResults = async (req: AuthRequest, res: Response) => {
  try {
    const { eventId } = req.params;
    const userId = req.user?.userId;
    ...
    const eventParticipations = participations.filter(p => p.quizId && (p.quizId as any).eventId._id.toString() === eventId);
